...
trapping <- read_xlsx("FIELDLIST_UA_BE_RESTOREID_2025 (2).xlsx") %>% 
  as.data.frame

colnames(trapping) <- trapping[1,]

trapping <- trapping[-1,]

dir_create("Intermediate")

write.csv(trapping, "Intermediate/TrappingUpdated.csv")

trapping <- read.csv("Intermediate/TrappingUpdated.csv")

trapping$weight..g.

trapping %>% head

Locations <- "Data/Spatial/RestoreID_Belgium_Location.xlsx" %>% read_xlsx

trapping %>% names

Locations <- 
  Locations %>% rename(Location = name_of_location)

trapping <- 
  trapping %>% 
  left_join(Locations %>% dplyr::select(Location, latitude, longitude),
            by = "Location")

# Everything will be a character lol
